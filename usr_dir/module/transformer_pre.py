...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, x, cond, diffusion_step, mask):
        ...
        for layer in self.in_layers:
            x = layer(x, cond=cond, diffusion_step=diffusion_step, spk=None, encoder_padding_mask=padding_mask, self_attn_mask=slf_attn_mask)
            in_x_collect.append(x)
            # self_attn_skip_collect.append(self_attn_skip_result)
            # ffn_skip_collect.append(ffn_skip_result)

        for layer in self.out_layers:
            x = layer(x, cond=cond, diffusion_step=diffusion_step, spk=None, encoder_padding_mask=padding_mask, self_attn_mask=slf_attn_mask, skip=in_x_collect.pop())
            # self_attn_skip_collect.append(self_attn_skip_result)
            # ffn_skip_collect.append(ffn_skip_result)
