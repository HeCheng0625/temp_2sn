...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x, conditioner, diffusion_step):
        # x: (B, residual_channel, T), diffusion_step: (B, residual_channel)
        diffusion_step = self.diffusion_projection(diffusion_step).unsqueeze(-1)
        conditioner = self.conditioner_projection(conditioner)   # (B, residual_channel*2, T)
        y = x + diffusion_step

        y = self.dilated_conv(y) + conditioner   # (B, residual_channel*2, T)
...
class WavNet(nn.Module):
    def __init__(self, in_dims=80):
        ...
        self.params = params = AttrDict(
            # Model params
            encoder_hidden=hparams["vqemb_size"],   # 256
            residual_layers=hparams['residual_layers'],   # 40
            residual_channels=hparams['residual_channels'],   # 512
            dilation_cycle_length=hparams['dilation_cycle_length'],   # 2
        )
    ...
    def forward(self, spec, diffusion_step, cond):
        ...
        for layer_id, layer in enumerate(self.residual_layers):
            x, skip_connection = layer(x, cond, diffusion_step)   # (B, residual_channel, T)
