...
class DiffusionTTS(nn.Module):
    def __init__(self, prior_encoder) -> None:
        ...
        self.priority_encoder = prior_encoder   # fs2_cln.FastSpeech2_dis
        
        self.quantizer = self.priority_encoder.quantizer   # Residual_VQ
        if hparams['diffusion_type'] == "default":
        ...
        elif hparams['diffusion_type'] == "edm":
            from .diffusion_edm import Diffusion
            self.diffusion = Diffusion(n_feats=hparams["vqemb_size"], dim=hparams["vqemb_size"], n_spks=hparams["num_spk"], spk_emb_dim=hparams['hidden_size'])
        elif hparams['diffusion_type'] == "velocity":
            from .diffusion_velocity import Diffusion
            self.diffusion = Diffusion(n_feats=hparams["vqemb_size"], dim=hparams["vqemb_size"], n_spks=hparams["num_spk"], spk_emb_dim=hparams['hidden_size'])
        else:
        ...
        if hparams['use_ref_enc']:
            ...
            arch = hparams['ref_enc_arch']   # "8 8 8 8 8 8"
    ...
    def forward(self, src_tokens, mel2ph, spk_embed=None,
        ...
        ref_spk_latent_mask = ref_latent_mask
        ...
        if hparams['use_ref_enc']:
            ...
            if hparams['ref_query_tokens'] > 0:
                query = self.query_emb(torch.arange(hparams['ref_query_tokens']).to(ref_mels_input.device)).repeat(B, 1, 1)   # (B, 32, C)
                if hparams['ref_query_norm']:
                    query = self.layer_norm(query)   
                query, _ = self.query_attn(query.transpose(0, 1), spk_embed.transpose(0,1), spk_embed.transpose(0,1), key_padding_mask=ref_latent_mask)   # Input: (T, B, C)
                query = query.transpose(0, 1)   # (B, T, C) = (B, 32, C)
                if hparams['predictor_type'] == 'sawavnet':
                    ...
                    is_empty = (torch.rand((B, 1, 1)) < hparams['spk_dropout']).repeat(1, hparams['ref_query_tokens'], 1)
                    is_empty = is_empty.to(spk_embed.device)
                    # use is_empty to replace spk_embed
        ...
        x_mask = (mel2ph == 0)
        # print(mel2ph, mel2ph.shape, ref_mels.shape)
        ...
        mu_x = mu_x.transpose(1, 2)   # B, C, T
        ref_mels = ref_mels.transpose(1, 2)   # B, C, T
        
        if hparams['ref_query_tokens'] > 0:
            spk_embed = query   # (B, 32, C)
            ref_latent_mask = torch.zeros(B, hparams['ref_query_tokens']).bool().to(query.device)
            
        if hparams['diffusion_type'] in ["default", "edm"]:            
            if hparams['diffusion_type'] == "default":
                x0_pred, noise_pred, noise_gt = self.diffusion.compute_loss(x0=ref_mels, x_mask=x_mask, mu=mu_x, spk=spk_embed, prompt=spk_embed if hparams['use_spk_prompt'] else reference_spk_latent, prompt_mask=ref_latent_mask if hparams['use_spk_prompt'] else ref_spk_latent_mask)   # prompt: query; spk is not used; x = x * (~x_mask)
                weight = 1.
            elif hparams['diffusion_type'] == "edm":
                x0_pred, noise_pred, noise_gt, weight = self.diffusion.compute_loss(x0=ref_mels, x_mask=x_mask, mu=mu_x, spk=spk_embed, prompt=spk_embed if hparams['use_spk_prompt'] else reference_spk_latent, prompt_mask=ref_latent_mask if hparams['use_spk_prompt'] else ref_spk_latent_mask)   # ref_latent_mask: (B, 32) * False (why? padding = 1)
                
            diff_out = {}
            diff_out["diff_x0_pred"] = x0_pred.transpose(1, 2)   # B, C, T -> B, T, C
            diff_out["diff_noise_pred"] = noise_pred.transpose(1, 2)
            diff_out["diff_noise_gt"] = noise_gt.transpose(1, 2)
            diff_out["weight"] = weight
            ...
            v_predict, v_gt, noise_gt, x0_predict = self.diffusion.compute_loss(x0=ref_mels, x_mask=x_mask, mu=mu_x, spk=spk_embed, prompt=spk_embed if hparams['use_spk_prompt'] else reference_spk_latent, prompt_mask=ref_latent_mask if hparams['use_spk_prompt'] else ref_spk_latent_mask)
            diff_out = {}
            diff_out["diff_v_pred"] = v_predict.transpose(1, 2)
            diff_out["diff_v_gt"] = v_gt.transpose(1, 2)
            diff_out["diff_noise_gt"] = noise_gt.transpose(1, 2)
            diff_out["diff_x0_pred"] = x0_predict.transpose(1, 2)
            diff_out["weight"] = 1.
    ...
    def infer_fs2(self, src_tokens, mel2ph, spk_embed=None,
        ...
        if self.ref_enc is not None:
            ...
            if hparams['ref_query_tokens'] > 0:
                ...
                # ani = 0
                # # print(query.shape)
                # n = hparams['ref_query_tokens']
                
                # cosi = torch.nn.CosineSimilarity(dim=0)
                # cos_sim = torch.zeros(n,n)
                # for i in range(n):
                #     for j in range(i+1, n):
                #         # print(query[0, i].shape)
                #         ani += cosi(query[0, i], query[0, j])
                #         cos_sim[i,j] = cosi(query[0, i], query[0, j])
                # print('ani:', ani *2 / (n * (n-1)))
                # print('ani matrix:', cos_sim)
                # exit()
                ...
                if hparams['predictor_type'] == 'sawavnet':
                    ...
                    query = self.ref_enc_out(query)
                if hparams['spk_dropout'] > 0.0:
                    empty_query = self.empty_query_emb(torch.arange(hparams['ref_query_tokens']).to(ref_mel_for_enc.device)).repeat(B, 1, 1)
                    empty_query = self.layer_norm(empty_query)
                    hparams['empty_query'] = empty_query
    ...
    def infer_diffusion(self, mu, n_timesteps, spk=None,temperature=1.0, ref_x=None, prompt=None, prompt_mask=None):
        ...
        if hparams['diffusion_type'] == "velocity":
            z = torch.randn(mu_x.shape[0], hparams['vqemb_size'], mu_x.shape[2]).to(mu_x) / temperature
        else:
            z = torch.randn(mu_x.shape[0], hparams['vqemb_size'], mu_x.shape[2]).to(mu_x) * hparams['noise_factor'] * hparams['sigma'] / temperature
