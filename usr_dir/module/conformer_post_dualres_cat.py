...
from modules.operations import OPERATIONS_DECODER, LayerNorm, MultiheadAttention, NewTransformerFFNLayer, TransformerFFNLayer, utils, ConformerConvLayer, ConformerFFNLayer, Linear

from .wavnet import AttrDict, SinusoidalPosEmb, Mish

class ConformerLayer(nn.Module):
    def __init__(self, c, num_heads, dropout, attention_dropout=0.1, relu_dropout=0.1, use_cln=False, need_skip=False):
        super().__init__()
        self.c = c
        self.dropout = dropout
        self.ffn1 = ConformerFFNLayer(c, 2 * c, dropout=relu_dropout)
        self.ffn2 = ConformerFFNLayer(c, 2 * c, dropout=relu_dropout)
        self.conv = ConformerConvLayer(dim=c, expansion_factor=2, kernel_size=31, dropout=relu_dropout)
        ...
        self.layer_norm4 = LayerNorm(c, use_cln=use_cln)
        self.need_skip=need_skip

        self.diffusion_projection = Linear(c, c)
        self.cond_projection = Linear(c, c)

        self.need_skip = need_skip

        if self.need_skip:
            self.skip_linear = Linear(c, c)
    ...
    def forward(
        ...
        if layer_norm_training is not None:
            ...
            self.layer_norm4.training = layer_norm_training
...
OPERATIONS_DECODER[13] = lambda c, dropout, use_cln, need_skip: ConformerLayer(c, diff_transformer_num_head, dropout=dropout,
...
class TransformerDecoder(nn.Module):
    def __init__(self, arch, hidden_size=None, dropout=None, use_cln=False):
        ...
        self.skip_projection = Linear(embed_dim, embed_dim)
    ...
    def forward(self, x, cond, diffusion_step, mask):
        ...
        for layer in self.in_layers:
            x, skip_result = layer(x, cond=cond, diffusion_step=diffusion_step, spk=None, self_attn_padding_mask=padding_mask, self_attn_mask=slf_attn_mask)
            in_x_collect.append(x)
            skip_collect.append(skip_result)

        for layer in self.out_layers:
            x, skip_result = layer(x, cond=cond, diffusion_step=diffusion_step, spk=None, self_attn_padding_mask=padding_mask, self_attn_mask=slf_attn_mask, skip=in_x_collect.pop())
            skip_collect.append(skip_result)
        ...
        # similar to wavnet
        x = torch.sum(torch.stack(skip_collect), dim=0) / sqrt(len(skip_collect))
...
class TransformerEstimator(nn.Module):
    def __init__(self, arch) -> None:
        ...
        self.mlp = nn.Sequential(
            Linear(dim, dim * 4),
            Mish(),
            Linear(dim * 4, params.transformer_hidden)
        )

        self.spec_linear = Linear(dim, params.transformer_hidden)
        self.cond_linear = Linear(params.condition_hidden, params.transformer_hidden)

        self.output_linear = Linear(params.transformer_hidden, dim)
        self.prompt_linear = Linear(dim, params.transformer_hidden)
        # self.prompt_linear = Linear(params.latent_dim, params.transformer_hidden)
