...
class DiffLayer(nn.Module):
    def __init__(self, c, num_heads, dropout, attention_dropout=0.1, relu_dropout=0.1, kernel_size=9, use_cln=False, need_skip=False):
        ...
        self.gru_projection_1 = nn.Linear(c, 2*c)
        self.gru_projection_2 = nn.Linear(c, 2*c)
    ...
    def forward(
        ...
        # gru
        x = self.gru_projection_1(x)
        gate, filter_ = torch.chunk(x, 2, dim=-1)
        x = torch.sigmoid(gate) * torch.tanh(filter_)
        x = self.gru_projection_2(x)
        x, ffn_skip_result = torch.chunk(x, 2, dim=-1)
...
class TransformerEstimator(nn.Module):
    ...
    def forward(self, spec, x_mask, diffusion_step, cond, spk, prompt, prompt_mask):
        # print("spec shape:", spec.shape, "x mask shape:", x_mask.shape, "diffusion step shape:", diffusion_step.shape,
        #         "cond shape", cond.shape, "prompt shape", prompt.shape, "prompt mask shape", prompt_mask.shape)
        # exit()
