...
from modules.operations import OPERATIONS_DECODER, NewLayerNorm, LayerNorm, MultiheadAttention, NewTransformerFFNLayer, TransformerFFNLayer, utils
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, x, cond, prompt, diffusion_step, mask):
        """
        :param x: [B, T, C]
        :param cond: [B, T, C]
        ...
        for layer in self.in_layers:
            x, self_attn_skip_result, ffn_skip_result = layer(x, cond=cond, prompt=prompt, diffusion_step=diffusion_step, spk=None, encoder_padding_mask=padding_mask, self_attn_mask=slf_attn_mask)
            in_x_collect.append(x)
            self_attn_skip_collect.append(self_attn_skip_result)
            ffn_skip_collect.append(ffn_skip_result)

        for layer in self.out_layers:
            x, self_attn_skip_result, ffn_skip_result = layer(x, cond=cond, prompt=prompt, diffusion_step=diffusion_step, spk=None, encoder_padding_mask=padding_mask, self_attn_mask=slf_attn_mask, skip=in_x_collect.pop())
            self_attn_skip_collect.append(self_attn_skip_result)
            ffn_skip_collect.append(ffn_skip_result)
...
class TransformerEstimator(nn.Module):
    def __init__(self, arch) -> None:
        ...
        # Use AdaptiveNorm for prompt in this version
    ...
    def forward(self, spec, x_mask, diffusion_step, cond, spk, prompt, prompt_mask):
        ...
        x = self.decoder(x=input, cond=cond, prompt=prompt, diffusion_step=step_emb, mask=mask_full)
