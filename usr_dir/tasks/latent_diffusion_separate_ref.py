...
class FastSpeech2Task(TransformerTtsTask):
    ...
    def build_model(self):
        ...
        ref_enc_count = self.get_model_stats(model.ref_enc) if hparams["use_ref_enc"] else 0
        ...
        # for idx, layer in enumerate(fs2_model.encoder.layers):
        #     print(layer, idx)
        #     print("It cost {} M".format(self.get_model_stats(layer) / 1000000))
            
        #     print("Self attention cost {} M".format(self.get_model_stats(layer.op.self_attn) / 1000000))
            
        #     print("ffn cost {} M".format(self.get_model_stats(layer.op.ffn) / 1000000))
        #     print("conv1d cost {} M".format(self.get_model_stats(layer.op.ffn.ffn_1) / 1000000))
        #     print("ffn2 cost {} M".format(self.get_model_stats(layer.op.ffn.ffn_2) / 1000000))
        #     exit(0)
        return model
    ...
    def _training_step(self, sample, batch_idx, _):
        ...
        ref_codes_mask = sample["ref_codes_mask"]
        
        # print(ref_codes.shape, target.shape, "11111", flush=True)
        # exit(0)
    ...
    def after_infer(self, predictions):
        ...
        for num_predictions, prediction in enumerate(t):
            ...
            gen_dir = os.path.join(hparams['work_dir'],
                                   f'generated_{self.trainer.global_step}_{hparams["gen_dir_name"]}_v3_tp{hparams["temp"]}'+ ('stoc' if hparams['stoc'] else '')+ ('_refnorm' if hparams['ref_norm'] else '')+('_inc' if hparams['in_context_infer'] else '' ))
