...
from .conformer_layer_wavnet import ConformerEncoderLayer
...
class S2TConformerEncoder(nn.Module):
    ...
    def forward(self, speech_latent, condition, diffusion_step, input_lengths, return_all_hiddens=False):
        ...
        for layer in self.conformer_layers:
            x, res, _ = layer(x, encoder_padding_mask, positions, diffusion_step=diffusion_step)
            encoder_states.append(res.unsqueeze(0))
            
        encoded_out = torch.cat(encoder_states, dim=0).sum(0) / sqrt(len(self.conformer_layers))

        return {
            "encoder_out": [encoded_out.transpose(0, 1)],  # B, T, C
