...
from .espnet_mha import RelPositionMultiHeadedAttention
...
class ConformerEncoderLayer(torch.nn.Module):
    ...
    def __init__(
        ...
        self.ffn2 = FeedForwardModule(
            ...
            dropout,
            activation_fn=activation_fn,
            output_feats=embed_dim * 2
        )
        self.final_layer_norm = LayerNorm(embed_dim, export=False)
        
        self.diffusion_projection = torch.nn.Linear(embed_dim, embed_dim)
    ...
    def forward(
        ...
        residual = x
        x = self.ffn1(x)
        x = x * 0.5 + residual + self.diffusion_projection(diffusion_step)
        residual = x
        ...
        residual = x
        x = self.ffn2(x)
        x, for_res = x.chunk(2, dim=-1)

        layer_result = x

        x = x * 0.5 + residual
        
        res = for_res * 0.5 + residual

        x = self.final_layer_norm(x)
        return x, res, (attn, layer_result)
