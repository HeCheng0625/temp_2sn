...
if __name__ == "__main__":
        ...
        for tar in tqdm(tars):
            ...
            for info in tar_obj:
                try:
                    if not info.isfile():
                        continue
                    if info.name.endswith(f'.speaker_id'):
                        hash_value = hashlib.sha256(
                            os.path.join(tar, info.name.replace('.speaker_id', '')).encode('utf-8')).hexdigest()
                        utt_ids.add(hash_value)
                        spk_id = int(info.name.split('.')[1].split('_')[0])
                        entry = {'tar': tar, 'fn': info.name.replace('.speaker_id', ''), 'utt_id': hash_value, 'spk_id': spk_id}
    ...
    left_utt_ids = list(utt_ids)
    random.shuffle(left_utt_ids)
    valid_num = 2000
    valid_utt_ids = left_utt_ids[:valid_num]
    valid_utt_ids = set(valid_utt_ids)
    train_utt_ids = utt_ids - valid_utt_ids
