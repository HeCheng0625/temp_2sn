...
from utils.preprocessor import process_utterance, get_mel2ph, get_pitch
...
def process_item(raw_data_dir, encoder, tg_fn, wav_fn):
    ...
    try:
        ...
        wav, mel = process_utterance(
            wav_fn, fft_size=hparams['n_fft'],
            hop_size=hparams['hop_size'], win_length=hparams['win_size'],
            num_mels=hparams['audio_num_mel_bins'],
            fmin=hparams['fmin'], fmax=hparams['fmax'], sample_rate=hparams['audio_sample_rate'],
            loud_norm=hparams['loud_norm'], min_level_db=hparams['min_level_db'],
    ...
    return item_name, phone_encoded, mel, mel2ph, spk_id, wav, pitch_coarse, f0, dur
...
def process_data(raw_data_dir, encoder, wav_fns, data_dir, prefix):
    ...
    for future in tqdm(futures):
        ...
        if res is None:
            continue
        item_name, phone_encoded, mel, mel2ph, spk_id, wav, pitch, f0, dur = res
...
if __name__ == "__main__":
    ...
    os.makedirs(hparams['data_dir'], exist_ok=True)
    process_data(raw_data_dir, encoder, all_wav_fns[:523], hparams['data_dir'], 'valid')
    process_data(raw_data_dir, encoder, all_wav_fns[:523], hparams['data_dir'], 'test')
    process_data(raw_data_dir, encoder, all_wav_fns[523:], hparams['data_dir'], 'train')
