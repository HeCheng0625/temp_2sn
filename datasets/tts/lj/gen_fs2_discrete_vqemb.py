...
def process_item(raw_data_dir, encoder, tg_fn, wav_fn, token, token_emb):
...
def process_data(raw_data_dir, encoder, wav_fns, tokens, data_dir, prefix):
    ...
    for wav_fn, token in zip(wav_fns, tokens):
        ...
        futures.append(p.apply_async(process_item, args=(raw_data_dir, encoder, item2tgfn[item_name], wav_fn, token, token_emb.cpu().numpy())))
    p.close()
