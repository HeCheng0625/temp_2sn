...
class StyleAdaptiveLayerNorm(nn.Module):
    def __init__(self, normalized_shape, eps=1e-5, elementwise_affine=True):
        ...
        self.norm = nn.LayerNorm(self.in_channel, elementwise_affine=False, eps=eps)   # y = (x - Ex) / sqrt(Var(x))
...
class NewStyleAdaptiveLayerNorm(nn.Module):
    def __init__(self, normalized_shape, eps=1e-5, elementwise_affine=True):
        super(NewStyleAdaptiveLayerNorm, self).__init__()
        self.in_channel = normalized_shape
        self.norm = nn.LayerNorm(self.in_channel, elementwise_affine=False, eps=eps)   # y = (x - Ex) / sqrt(Var(x))
    ...
    def forward(self, input, condition):
        # condition: (32, B, 512)
        style = self.style(torch.sum(condition, dim=0)).unsqueeze(0)
...
def NewLayerNorm(normalized_shape, eps=1e-5, elementwise_affine=True, export=False, use_cln=False):
    
    if use_cln:
        return NewStyleAdaptiveLayerNorm(normalized_shape=normalized_shape, eps=eps, elementwise_affine=elementwise_affine)
    else:
        return ClassicLayerNorm(normalized_shape=normalized_shape, eps=eps, elementwise_affine=elementwise_affine)
...
class MultiheadAttentionIndependentW(nn.Module):
    def __init__(self, embed_dim, num_heads, kdim=None, vdim=None, dropout=0., bias=True,
                 add_bias_kv=False, add_zero_attn=False, self_attention=False, query_token=1,
                 encoder_decoder_attention=False):
        super().__init__()
        self.embed_dim = embed_dim
        self.kdim = kdim if kdim is not None else embed_dim
        self.vdim = vdim if vdim is not None else embed_dim
        self.qkv_same_dim = False # = self.kdim == embed_dim and self.vdim == embed_dim
        ...
        if self.qkv_same_dim:
            ...
            self.k_proj_weight = Parameter(torch.Tensor(query_token, embed_dim, self.kdim))
            self.v_proj_weight = Parameter(torch.Tensor(query_token, embed_dim, self.vdim))
            self.q_proj_weight = Parameter(torch.Tensor(query_token, embed_dim, embed_dim))
        ...
        self.out_proj_weight = Parameter(torch.Tensor(query_token, embed_dim, embed_dim))
        
        self.out_proj_bias = Parameter(torch.Tensor(query_token, 1, embed_dim))
    ...
    def forward(
        ...
        # query: [query_num, B, d] project_w: [query_num, D, D]
        
        query = torch.einsum("qbd,qdd->qbd", query, self.q_proj_weight).contiguous()
        
        query = query.view(1, self.query_token * bsz, embed_dim)
        
        # key: [seq_len, B, d], prj_w: [query_token, d, d]
        key = torch.einsum("sbd,qdd->sqbd", key, self.k_proj_weight).contiguous()
        
        value = torch.einsum("sbd,qdd->sqbd", value, self.v_proj_weight).contiguous()
        
        key = key.view(src_len, self.query_token * bsz, embed_dim)
        value = value.view(src_len, self.query_token * bsz, embed_dim)
        ...
        key_padding_mask = key_padding_mask.unsqueeze(0).expand(self.query_token, -1, -1).contiguous().view(bsz, src_len)
        ...
        out_proj_weight = self.out_proj_weight.unsqueeze(1).expand(self.query_token, old_bsz, self.embed_dim, self.embed_dim).contiguous().view(self.query_token * old_bsz, self.embed_dim, self.embed_dim)
        
        attn = torch.einsum("sbd,bdd->sbd", attn, out_proj_weight)
        
        # bias: [query_token, 1, dim] -> [query_token * bsz, dim]
        bias = self.out_proj_bias.expand(-1, old_bsz, -1).contiguous().view(1, bsz, self.embed_dim)
        
        attn = attn + bias # [1, q*bsz, d]
        
        attn = attn.view(self.query_token, old_bsz, self.embed_dim)
...
class EncLocalSALayer(nn.Module):
    ...
    def forward(self, x, encoder_padding_mask=None, **kwargs):
        ...
        for i in range(0, x.shape[0], half_chunk_size + 1):
            ...
            q_nonpadding = (1 - encoder_padding_mask[:, i:i + half_chunk_size + 1].to(x.dtype)).data
...
class DecConvLayer(nn.Module):
    ...
    def forward(self, x, encoder_out=None, encoder_padding_mask=None, incremental_state=None, **kwargs):
        ...
        x, attn = self.attention(
            ...
            static_kv=True,
            enc_dec_attn_constraint_mask=utils.get_incremental_state(self, incremental_state,
...
class VanillaFFNLayer(nn.Module):
    def __init__(self) -> None:
        super().__init__()

class kaiDecSALayer(nn.Module):
    def __init__(self, c, num_heads, dropout, attention_dropout=0.1, relu_dropout=0.1, kernel_size=9, use_cln=False):
        super().__init__()
        self.c = c
        self.dropout = dropout
        self.layer_norm1 = LayerNorm(c, use_cln=use_cln)
    ...
    def forward(
        ...
        x = self.layer_norm3(x, condition=condition)
        # x = self.ffn(x, incremental_state=incremental_state)
        x = F.relu(self.fc1(x))
        x = F.dropout(x, self.dropout, training=self.training)
        x = self.fc2(x)
...
class LSTMAttentionLayer(nn.Module):
    ...
    def forward(self, input, source_hids, encoder_padding_mask=None, enc_dec_attn_constraint_mask=None):
        ...
        # don't attend over padding
        if encoder_padding_mask is not None:
            attn_weights = attn_weights.to(x.dtype).masked_fill_(
                encoder_padding_mask.unsqueeze(1),
                float('-inf')
            ).type_as(attn_weights)  # FP16 support: cast to float and back
...
OPERATIONS_ENCODER = {  # c = hidden size
    ...
    8: lambda c, dropout, use_cln: EncSALayer(c, 4, use_cln=use_cln, dropout=dropout,
    ...
    16: lambda c, dropout: EncSALayer(c, 8, dropout=dropout,
                                     attention_dropout=0.0, relu_dropout=dropout,
                                     kernel_size=hparams['enc_ffn_kernel_size'],
                                     padding=hparams['ffn_padding']),
}
...
OPERATIONS_DECODER = {
    ...
    14: lambda c, dropout, use_cln: kaiDecSALayer(c, 2, use_cln=use_cln, dropout=dropout,
                                     attention_dropout=0.0, relu_dropout=dropout,
                                     kernel_size=hparams['dec_ffn_kernel_size']),
}
