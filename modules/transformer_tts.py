...
class TransformerEncoder(nn.Module):
    def __init__(self, arch, embed_tokens, last_ln=True, use_cln=False):
        super().__init__()
        self.arch = arch
    ...
    def forward(self, src_tokens, condition=None):
        """
        ...
        for layer in self.layers:
            x = layer(x, encoder_padding_mask=encoder_padding_mask, condition=condition)
...
class TransformerDecoder(nn.Module):
    ...
    def forward_prenet(self, x):
        mask = x.abs().sum(-1, keepdim=True).ne(0).to(x.dtype)
