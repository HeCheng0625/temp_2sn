...
class TransformerTtsTask(BaseTask):
    ...
    def validation_step(self, sample, batch_idx):
        ...
        outputs['focus_rate'] = utils.get_focus_rate(encdec_attn, src_padding_mask, target_padding_mask).mean()
        outputs['phone_coverage_rate'] = utils.get_phone_coverage_rate(encdec_attn, src_padding_mask, src_seg_mask,
                                                                       target_padding_mask).mean()
        attn_ks = src_lengths.float() / target_lengths.float()
        outputs['diagonal_focus_rate'], diag_mask = utils.get_diagonal_focus_rate(encdec_attn, attn_ks,
                                                                                  target_lengths,
                                                                                  src_padding_mask,
                                                                                  target_padding_mask)
