...
from modules.fs2_dis import FastSpeech2_dis
...
class FastSpeechDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        vq_emb = torch.Tensor(item['vq_emb'][:hparams['max_frames']])
...
class FastSpeech2Task(TransformerTtsTask):
    def __init__(self):
        ...
        # vocoder.load_state_dict(torch.load("/blob/v-shenkai/checkpoints/tts/codec/chanpin_5w/v5/lambda_disc_1_commit_weight_0.25/infered_lj_440000/generator_hop200.pt", map_location=lambda loc, storage: loc))
        # _ = vocoder.cuda().eval()
    ...
    def build_model(self):
        arch = self.arch
        model = FastSpeech2_dis(arch, self.phone_encoder)
        return model

    def _training_step(self, sample, batch_idx, _):
        input = sample['src_tokens']  # [B, T_t]
        target = sample['targets']  # [B, T_s, 80]
        # target_lengths = sample['target_lengths']  # [B]
    ...
    def run_model(self, model, input, mel2ph, spk_embed, target,
        ...
        if "pred_pitch_after" in hparams and hparams["pred_pitch_after"] >= 0:
            output = model(input, mel2ph, spk_embed, ref_mel, pitch, uv, energy, use_pred_pitch=self.global_step > hparams["pred_pitch_after"])
        else:
        ...
        # if hparams.get("vqemb_predict", False):
        losses['mel'] = self.l1_loss(output['mel_out'], target)
        # if hparams.get("parallel_predict", False):
        #     #target = target - 1  # remove pad
        #     to_modify = (torch.arange(target.shape[-1], device=target.device) * hparams["codebook_size"])[None, None, :]
        #     target = (target - to_modify).clamp(min=0)

        
        # losses['mel'], mel_acc = self.ce_loss(output['mel_out'], target[:, :, :hparams["predict_first_ndim"]])
        # output["mel_acc"] = mel_acc
    ...
    def test_step(self, sample, batch_idx):
        ...
        # _, codes, _ = self.model.quantizer(outputs['mel_out'].transpose(1, 2))
        # sample['codes'] = codes
        # print(codes.shape, outputs['mel_out'].shape)
        if self.vocoder is None:
            ...
            _ = vocoder.cuda().eval()
            self.vocoder = vocoder
        y_dec_quant, _, _ = self.vocoder(outputs['mel_out'].transpose(1, 2), None, 100000, vq=True, early_stop=-1)
        audio, _ = self.vocoder.inference(y_dec_quant)
        sample['audio'] = audio
    ...
    def after_infer(self, predictions):
        ...
        for num_predictions, prediction in enumerate(t):
            ...
            # assert outputs.shape[0] %  hparams["predict_first_ndim"] == 0
            # time_length = int(outputs.shape[0] / hparams["predict_first_ndim"])
            # if hparams.get("parallel_predict", False):
            #     #target = target - 1  # remove pad
            #     to_modify = (torch.arange(hparams["predict_first_ndim"], device=outputs.device) * hparams["codebook_size"])[None, :].repeat(time_length, 1)
            #     outputs_id = outputs.max(-1)[1] - 1
            #     outputs_id = outputs_id + to_modify.reshape(-1)
            #     outputs_id = outputs_id.numpy()
            # else:
            #     is_valid = torch.arange(hparams["predict_first_ndim"], device=outputs.device)

            #     is_valid = is_valid * hparams["codebook_size"] + 1
            #     is_valid = is_valid[None, :].repeat(time_length, 1).reshape(-1)[:, None]
            #     is_valid_arange = torch.arange(outputs.shape[1], device=outputs.device)[None, :]
            #     is_valid = (is_valid_arange >= is_valid) * (is_valid_arange < is_valid + hparams["codebook_size"])

            #     outputs = outputs.masked_fill(~is_valid, float("-inf"))

            #     outputs_id = outputs.max(-1)[1].numpy() - 1  # 0 used for padding, 1 for the first token

            # print_str = " ".join(
            #     list(map(lambda x: "sp" + str(x), list(outputs_id)))
            # )
            
            # print_str = print_str + "prompt5" + print_str
            ...
            print(gen_dir)
            sf.write(f'{gen_dir}/{os.path.basename(utt_id)}.wav', audio[0], 16000, subtype='PCM_24')
            exit(0) 
