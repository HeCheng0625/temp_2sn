...
# --config configs/tts/product/latent_gpt_5wdata.yaml
python -m usr_dir.tasks.uni_speech_gpt_pl2 --config configs/tts/lj/fs2_latent_gpt.yaml --exp_name ${SAVE_DIR} --reset --hparams "data_dir=${DATA_DIR},raw_data_dir=${TEXT},max_tokens=8000,log_interval=1,\
            strategy=deepspeed,accumulate_grad_batches=20,val_check_interval=1000,lr=5e-4,\
