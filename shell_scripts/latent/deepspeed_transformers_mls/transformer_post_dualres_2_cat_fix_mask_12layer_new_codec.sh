...
SAVE_DIR=/blob/v-yuancwang/ns2/checkpoints/transformers/transformer_post_dualres_2_cat_fix_mask_12layer_new_codec_spk_prompt_false
# /bin/rm -rf ${SAVE_DIR}
...
python -m usr_dir.tasks.latent_diffusion_pl2 --config configs/tts/product/latent_diffusion_5wdata.yaml --exp_name ${SAVE_DIR} --reset --hparams "data_dir=${DATA_DIR},\
                                            raw_data_dir=${TEXT},max_tokens=9600,detach_mu=${detach_mu},detach_wavenet=${detach_wavenet},\
