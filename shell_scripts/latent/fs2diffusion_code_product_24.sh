...
SAVE_DIR=/blob/v-zeqianju/code/tts/fs2_fromyc/mm_lm_nat/checkpoints/product_24_latent_fs2diffusion_code

# /bin/rm -rf ${SAVE_DIR}

mkdir -p ${SAVE_DIR}
#export CUDA_VISIBLE_DEVICES=0
python -m usr_dir.tasks.latent_diffusion --config configs/tts/lj/fs2_latent.yaml --exp_name ${SAVE_DIR} --reset --hparams "data_dir=${DATA_DIR},raw_data_dir=${TEXT},max_tokens=16000,diffusion_mel_weight=1,diff_loss_noise_weight=1,vq_quantizer_weight=1,vq_dist_weight=1,log_interval=100,max_frames=1000,max_input_tokens=300,max_tokens=32000"
