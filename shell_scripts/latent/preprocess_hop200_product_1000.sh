TRAIN_FILE=/blob/v-zeqianju/dataset/tts/product_5w/1000_h.txt
VALID_FILE=/blob/v-zeqianju/dataset/tts/product_5w/valid.txt
TEST_FILE=/blob/v-zeqianju/dataset/tts/product_5w/test.txt
CODE_DATA=/blob/v-zeqianju/dataset/tts/product_5w/1000h_code
EMB_DATA=/blob/v-zeqianju/dataset/tts/product_5w/1000h_emb
...
python -m datasets.tts.product.gen_fs2_discrete_vqemb_hop200res16 --hparams "data_dir=${DATA_DIR},raw_data_dir=${TEXT},vq_ckpt=${VQ_CKPT},train_file=${TRAIN_FILE},valid_file=${VALID_FILE},test_file=${TEST_FILE},code_data_dir=${CODE_DATA},emb_data_dir=${EMB_DATA},f0_use_product=False" --config configs/tts/product/fs2_discrete_vqemb_hop200res16.yaml
