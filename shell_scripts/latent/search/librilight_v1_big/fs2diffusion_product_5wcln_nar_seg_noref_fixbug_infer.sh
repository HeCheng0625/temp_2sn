...
save_prefix=product_5w_v1.1big_segnoref_per4
...
prior_use_cln=False
diffusion_ca_per_layer=4
predictor_use_cattention=True
predictor_ca_per_layer=4
...
python -m usr_dir.tasks.latent_diffusion_separate_ref --config configs/tts/product/latent_diffusion_5wdata.yaml --exp_name ${SAVE_DIR} --reset --hparams "data_dir=${DATA_DIR},raw_data_dir=${TEXT},max_tokens=6000,detach_mu=${detach_mu},detach_wavenet=${detach_wavenet},diffusion_mel_weight=${diffusion_mel_weight},diff_loss_noise_weight=${diff_loss_noise_weight},vq_quantizer_weight=${vq_quantizer_weight},vq_dist_weight=${vq_dist_weight},dilation_cycle_length=${dilation_cycle_length},pe_scale=${pe_scale},use_spk_embed=True,enc_layers=${enc_layers},dec_layers=${dec_layers},arch=${arch},residual_channels=${residual_channels},residual_layers=${residual_layers},hidden_size=${hidden_size},max_updates=100000000,\
                                            ...
                                            lr=0.1,max_frames=3000,max_input_tokens=600,use_random_segment_as_ref=True,\
                                            use_ref_enc=${use_ref_enc},ref_enc_arch=${ref_enc_arch},skip_decoder=True,\
                                            diffusion_ca_per_layer=${diffusion_ca_per_layer},predictor_use_cattention=${predictor_use_cattention},predictor_ca_per_layer=${predictor_ca_per_layer},\
                                            train_set_name=valid,test_set_name=${test_set_name},infer_laoban=${infer_laoban},in_context_infer=${in_context_infer},stoc=${stoc},temp=${temp},ref_norm=${ref_norm}" --infer
