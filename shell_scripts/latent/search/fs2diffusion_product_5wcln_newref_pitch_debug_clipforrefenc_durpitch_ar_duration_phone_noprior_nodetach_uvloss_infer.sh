...
save_prefix=product_5w_400MARDurationPitch_noprior_nodetach_hasuv_phone
reference_encoder_filters="32 64 128 256 256 512 512"
arch='8 8 8 8 8 8 8 8 8 8'
enc_layers=10
dec_layers=0
residual_channels=512
residual_layers=60
hidden_size=512
pitch_predictor_n_layers=8
...
pitch_predictor_hidden=768
spk_dropout=0.0
ref_random_clip=True
dur_cln=True
pitch_cln=True
...
# duration parameters
duration_transformer_arch="14 14 14 14 14 14 14 14"
pitch_transformer_arch="14 14 14 14 14 14 14 14"
...
python -m usr_dir.tasks.latent_diffusion --config configs/tts/product/latent_diffusion_5wdata.yaml --exp_name ${SAVE_DIR} --reset --hparams "data_dir=${DATA_DIR},raw_data_dir=${TEXT},max_tokens=13000,detach_mu=${detach_mu},detach_wavenet=${detach_wavenet},diffusion_mel_weight=${diffusion_mel_weight},diff_loss_noise_weight=${diff_loss_noise_weight},vq_quantizer_weight=${vq_quantizer_weight},vq_dist_weight=${vq_dist_weight},dilation_cycle_length=${dilation_cycle_length},pe_scale=${pe_scale},use_ref_enc=True,use_spk_embed=True,enc_layers=${enc_layers},dec_layers=${dec_layers},arch=${arch},residual_channels=${residual_channels},residual_layers=${residual_layers},hidden_size=${hidden_size},max_updates=100000000,\
                                            use_amp=True,accumulate_grad_batches=8,log_interval=100,prior_use_cln=True,diffusion_use_film=True,use_pitch_embed=${use_pitch_embed},predictor_type=sawavnet,diffusion_has_sattn=False,reference_encoder_filters=${reference_encoder_filters},remove_bos=True,use_new_refenc=${use_new_refenc},predictor_hidden=${predictor_hidden},spk_dropout=${spk_dropout},ref_random_clip=${ref_random_clip},dur_cln=${dur_cln},pitch_cln=${pitch_cln},\
                                            duration_predictor_type=ar_transformer,duration_transformer_arch=${duration_transformer_arch},\
                                            pitch_predictor_type=ar_transformer_phone,pitch_transformer_arch=${pitch_transformer_arch},\
                                            predictor_sg=${predictor_sg},\
                                            diffusion_from_prior=${diffusion_from_prior},prior_weight=${prior_weight},test_set_name=${test_set_name},train_set_name=valid,infer_laoban=${infer_laoban},in_context_infer=${in_context_infer}" --infer
