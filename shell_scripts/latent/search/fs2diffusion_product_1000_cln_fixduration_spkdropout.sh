...
spk_dropout=0.2
save_prefix=product_1000_conditionLNfilmfixduration_spkdropout${spk_dropout}
...
python -m usr_dir.tasks.latent_diffusion --config configs/tts/lj/fs2_latent.yaml --exp_name ${SAVE_DIR} --reset --hparams "data_dir=${DATA_DIR},raw_data_dir=${TEXT},max_tokens=20000,detach_mu=${detach_mu},detach_wavenet=${detach_wavenet},diffusion_mel_weight=${diffusion_mel_weight},diff_loss_noise_weight=${diff_loss_noise_weight},vq_quantizer_weight=${vq_quantizer_weight},vq_dist_weight=${vq_dist_weight},dilation_cycle_length=${dilation_cycle_length},pe_scale=${pe_scale},use_ref_enc=True,use_spk_embed=True,enc_layers=${enc_layers},dec_layers=${dec_layers},arch=${arch},residual_channels=${residual_channels},hidden_size=${hidden_size},max_updates=100000000,\
                                            use_amp=True,accumulate_grad_batches=2,log_interval=100,prior_use_cln=True,diffusion_use_film=True,use_pitch_embed=False,predictor_type=sawavnet,diffusion_has_sattn=False,remove_bos=True,spk_dropout=${spk_dropout}" 2>&1 | tee -a ${SAVE_DIR}/train.log
