...
save_prefix=product_5w_conditionLNfilm_newref_transformer_pitch_noprior_fixbug
...
diffusion_sa_per_layer=4
skip_decoder=False
diffusion_from_prior=False
prior_weight=0.0
transformer_hidden=512
transformer_esitimator_arch='13 13 13 13 13 13 13 13'
dur_cln=True
pitch_cln=True
...
python -m usr_dir.tasks.latent_diffusion --config configs/tts/product/latent_diffusion_5wdata.yaml --exp_name ${SAVE_DIR} --reset --hparams "data_dir=${DATA_DIR},raw_data_dir=${TEXT},max_tokens=15000,detach_mu=${detach_mu},detach_wavenet=${detach_wavenet},diffusion_mel_weight=${diffusion_mel_weight},diff_loss_noise_weight=${diff_loss_noise_weight},vq_quantizer_weight=${vq_quantizer_weight},vq_dist_weight=${vq_dist_weight},dilation_cycle_length=${dilation_cycle_length},pe_scale=${pe_scale},use_ref_enc=True,use_spk_embed=True,enc_layers=${enc_layers},dec_layers=${dec_layers},arch=${arch},residual_channels=${residual_channels},residual_layers=${residual_layers},hidden_size=${hidden_size},max_updates=100000000,\
                                            use_amp=True,accumulate_grad_batches=6,log_interval=100,prior_use_cln=True,diffusion_use_film=True,use_pitch_embed=${use_pitch_embed},predictor_type=transformer,transformer_hidden=${transformer_hidden},transformer_esitimator_arch=${transformer_esitimator_arch},diffusion_has_sattn=${diffusion_has_sattn},reference_encoder_filters=${reference_encoder_filters},remove_bos=True,use_new_refenc=${use_new_refenc},predictor_hidden=${predictor_hidden},spk_dropout=${spk_dropout},diffusion_sa_per_layer=${diffusion_sa_per_layer},skip_decoder=${skip_decoder},diffusion_from_prior=${diffusion_from_prior},prior_weight=${prior_weight},dur_cln=${dur_cln},pitch_cln=${pitch_cln}" 2>&1 | tee -a ${SAVE_DIR}/train.log
