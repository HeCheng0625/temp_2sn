...
SAVE_DIR=/blob/v-zeqianju/code/tts/fs2_fromyc/mm_lm_nat/checkpoints/ljspeech_discrete_44wres16hop200_vqemb_noise_factor${noise_factor}

mkdir -p ${SAVE_DIR}
python -m tasks.fs2_discrete_vqemb --config configs/tts/lj/fs2_discrete_vqemb_hop200res16.yaml --exp_name ${SAVE_DIR} --reset --hparams "data_dir=${DATA_DIR},raw_data_dir=${TEXT},max_tokens=10000,noise_factor=${noise_factor}"
