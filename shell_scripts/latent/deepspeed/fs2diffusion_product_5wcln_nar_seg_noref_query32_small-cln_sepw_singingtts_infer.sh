...
save_prefix=singingtts_full_ft_product_5w_v1.1big_segnoref_query32_small-cln_refnorm_sepw
# save_prefix=debug
...
noise_factor=3
load_opt=False
SAVE_DIR=/blob/v-yuancwang/LATENT_TTS/finetune_11lab
...
python -m usr_dir.tasks.latent_diffusion_pl2 --config configs/tts/product/latent_diffusion_5wdata.yaml --exp_name ${SAVE_DIR} --reset --hparams "data_dir=${DATA_DIR},\
                                            raw_data_dir=${TEXT},max_tokens=10000,detach_mu=${detach_mu},\
                                            detach_wavenet=${detach_wavenet},diffusion_mel_weight=${diffusion_mel_weight},\
                                            diff_loss_noise_weight=${diff_loss_noise_weight},vq_quantizer_weight=${vq_quantizer_weight},\
                                            vq_dist_weight=${vq_dist_weight},dilation_cycle_length=${dilation_cycle_length},pe_scale=${pe_scale},use_spk_embed=True,enc_layers=${enc_layers},dec_layers=${dec_layers},arch=${arch},residual_channels=${residual_channels},residual_layers=${residual_layers},hidden_size=${hidden_size},max_updates=100000000,\
                                            use_amp=True,log_interval=100,prior_use_cln=${prior_use_cln},diffusion_use_film=True,\
                                            use_pitch_embed=${use_pitch_embed},diffusion_has_sattn=False,reference_encoder_filters=${reference_encoder_filters},remove_bos=True,use_new_refenc=${use_new_refenc},predictor_hidden=${predictor_hidden},spk_dropout=${spk_dropout},ref_random_clip=${ref_random_clip},dur_cln=${dur_cln},pitch_cln=${pitch_cln},duration_layers=${duration_layers},pitch_layers=${pitch_layers},diffusion_from_prior=${diffusion_from_prior},prior_weight=${prior_weight},predictor_use_res=${predictor_use_res},\
                                            strategy=ddp,accumulate_grad_batches=1,num_nodes=1,warmup_updates=3000,\
                                            lr=5e-5,max_frames=3000,max_input_tokens=600,use_random_segment_as_ref=True,val_check_interval=100,\
                                            ...
                                            ref_query_norm=${ref_query_norm},ref_query_tokens=${ref_query_tokens},predictor_use_cattention=${predictor_use_cattention},predictor_ca_per_layer=${predictor_ca_per_layer},use_spk_prompt=${use_spk_prompt},\
                                            profile_infer=True,infer_style=${infer_style},n_samples=${n_samples},inference_ckpt=${inference_ckpt},train_set_name=valid,test_set_name=${test_set_name},infer_laoban=${infer_laoban},in_context_infer=${in_context_infer},stoc=${stoc},temp=${temp},ref_norm=${ref_norm}" --infer
