...
logger_nest = logging.getLogger(__name__)
        ...
        if self.epochs_since_last_check >= self.period:
            ...
            if current is not None:
                if self.monitor_op(current, self.best):
                    ...
                    trainer.save_checkpoint(best_filepath)
...
class BaseTrainer:
    def __init__(
        ...
        if self.use_amp:
            ...
            logger_nest.info('Using native 16bit precision.')
        else:
            self.scaler = None
        print(use_amp)
        # exit()
    ...
    def run_training_epoch(self):
        ...
        for batch_idx, batch in enumerate(self.get_train_dataloader()):
            ...
            # ---------------
            should_check_val = (
                    not self.disable_validation and self.global_step % (self.val_check_batch * self.accumulate_grad_batches) == 0 and not self.fisrt_epoch)
            ...
            should_log_metrics = batch_idx % (self.row_log_interval * self.accumulate_grad_batches) == 0 or early_stop_epoch
            ...
            if should_log_metrics:
                ...
                msg = "[Epoch {}/{}, step {}/{}] ".format(self.current_epoch, self.min_epochs, self.global_step // self.accumulate_grad_batches, self.total_batches // self.accumulate_grad_batches)
                
                for k, v in batch_step_metrics.items():
                    if k == "tr/batch_size":
                        msg += "{}: {:.4f}, ".format(k.replace("tr/", ""), v * self.accumulate_grad_batches)
                    else:
                        msg += "{}: {:.4f}, ".format(k.replace("tr/", ""), v)
                logger_nest.info(msg)
            ...
            for opt_idx, optimizer in enumerate(self.optimizers):
                ...
                def optimizer_closure():
                    ...
                    if closure_loss.requires_grad:
                        ...
                        if self.use_amp:
                            self.scaler.scale(closure_loss).backward()  # ugly modification
                        else:
                ...
                if (self.batch_idx + 1) % self.accumulate_grad_batches == 0:
                    ...
                    model = self.get_model()
                    model.optimizer_step(self.current_epoch, batch_idx, optimizer, opt_idx, scalar=self.scaler, use_amp=self.use_amp)
